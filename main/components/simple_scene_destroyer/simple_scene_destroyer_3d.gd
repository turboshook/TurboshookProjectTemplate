extends Node
class_name SimpleSceneDestroyer3D

@export_category("Dependencies")
@export var scene: Node3D
@export var visual_representation: Node3D
...
func _ready() -> void:
	if not scene:
		push_warning("SimpleSceneDestroyer3D @ _ready(): no scene provided!")
	if not visual_representation:
		push_warning("SimpleSceneDestroyer3D @ _ready(): no visual_representation provided!")
